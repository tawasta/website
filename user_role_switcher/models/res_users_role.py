...
class ResUsersRole(models.Model):
    ...
    def create(self, vals):
        ...
        if "locked_for_allowed_roles" in vals and not self.env.user.has_group(
            "base.group_system"
        ):
            raise AccessError(
                _(
                    "Only system administrators can set 'Locked for Allowed/Current Role'."
                )
            )
    ...
    def write(self, vals):
        ...
        if "locked_for_allowed_roles" in vals and not self.env.user.has_group(
            ...
            raise AccessError(
                _(
                    "Only system administrators can modify 'Locked for Allowed/Current Role'."
                )
            )
