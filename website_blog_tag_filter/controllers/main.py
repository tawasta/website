...
class BlogTagController(http.Controller):
    @http.route(
        ["/tag/<int:tag_id>", "/tag/<int:tag_id>/page/<int:page>"],
        type="http",
        auth="public",
        website=True,
    )
    def blog_posts_by_tag(self, tag_id, page=1, **kwargs):
        ...
        pager = portal_pager(url=f"/tag/{tag_id}", total=total, page=page, step=10)

        posts = BlogPost.search(
            domain, order=sort_order, limit=10, offset=pager["offset"]
        )
        tag = request.env["blog.tag"].sudo().browse(tag_id)

        return request.render(
            "website_blog_tag_filter.blog_posts_by_tag",
            {
                "posts": posts,
                "pager": pager,
                "tag": tag,
