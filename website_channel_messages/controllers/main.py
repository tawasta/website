...
def process_message(user, record, data, **kwargs):
    ...
    if comment:
        ...
        if not error:
            notified_partner_ids = record.channel_member_ids.mapped(
                "partner_id"
            ).ids
...
class WebsiteChannelMessagesController(http.Controller):
    ...
    def website_channels(self, search="", page=1, **post):
        ...
        channel_model = request.env["discuss.channel"]
        ...
        channels = (
            ...
            .sorted(key=lambda r: r.message_ids.ids, reverse=True)
        )
    ...
    def channel_create(self, recipients, csrf_token):
        ...
        """
        current_user = request.env.user
        MailChannel = request.env["discuss.channel"]
        values = {}
