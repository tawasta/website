...
class VisitorImageCategory(models.Model):
    _name = 'imagebank.category'
    name = fields.Char(string='Name')

class WebsiteVisitorImage(models.Model):
    _name = 'imagebank.image'
    _description = 'Image uploaded by website visitor'

    name = fields.Char(string="Name")
    category = fields.Many2one(string="Category", comodel_name='imagebank.category', ondelete='restrict')
    ...
    attachment_thumbnail = fields.Binary(related="attachment.thumbnail", attachment=True)

    def get_category_list(self):
        categories = []
        for category in self.env['imagebank.category'].search([]):
