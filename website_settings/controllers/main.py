...
class SocietyHome(home):
    ...
    def web_login(self, redirect=None, *args, **kw):
        response = super(SocietyHome, self).web_login(
            redirect=redirect, *args, **kw)
        if not redirect and request.params['login_success']:
            if request.website.sudo().landing_page_group_text or request.website.sudo().landing_page:
                get_param = request.env["ir.config_parameter"].sudo().get_param
                group_param = get_param("group.landing.page")
                is_in_group = request.env["res.groups"].sudo().search([
                    ('id', '=', group_param),
